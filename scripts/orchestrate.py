...
def main() -> None:  # noqa: C901 — намеренно монолитная, чтобы поток был виден
    ...
    parser.add_argument("--build-index", action="store_true",
                        help="Перед экспериментами построить ChromaDB-коллекции")
    parser.add_argument("--index-only", action="store_true",
                        help="Только построить ChromaDB-коллекции, не запускать эксперименты")
    ...
    all_experiments: list[dict] = grid_cfg.get("experiments", [])
    if args.experiments:
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # ШАГ 1: Построить ChromaDB-коллекции (если нужно)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    if args.build_index or args.index_only:
        ...
        print(f"\n[INDEX] Нужно построить {len(seen_idx)} уникальных ChromaDB-коллекций")
        ...
        for fp, exp in seen_idx.items():
            ...
            if not args.force and not args.no_resume:
                if entry.get("status") == "done" and entry_ok(entry):
                    print(f"[SKIP]  Index {coll_name}  (fingerprint={fp}, уже построен)")
                    continue
