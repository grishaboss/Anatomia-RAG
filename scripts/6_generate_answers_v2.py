...
#    )
    ...
    return (
        "# ИНСТРУКЦИЯ:\n"
        "Ты — редактор учебного конспекта по анатомии человека. "
        "Твоя задача: найти в предоставленных фрагментах учебников всё, что относится "
        "к вопросу и каждому аспекту, и организовать ответ близко к тексту источников.\n\n"

        "# КАТЕГОРИЧЕСКИЕ ПРАВИЛА:\n"
        "1. Используй ИСКЛЮЧИТЕЛЬНО текст из фрагментов ниже — не добавляй никаких фактов, "
        "цифр или терминов, которых нет в этих текстах.\n"
        "2. Если по какому-то аспекту информации в источниках нет — напиши: [нет в источниках].\n"
        "3. Если Гайворонский и Сапин описывают одно и то же — пиши один раз без атрибуции.\n"
        "4. Если авторы расходятся: разные формулировки, цифры или акценты — укажи явно: "
        ...
        "Напиши единый связный ответ на экзаменационный вопрос выше на 3–4 абзаца, "
        "примерно для ответа на 5 минут. "
        "Используй аспекты как ориентиры для полноты охвата — каждый из них должен быть "
        "раскрыт в тексте, но НЕ оформляй их как отдельные пункты с заголовками. "
        "Выписывай близко к тексту источников, можешь суммаризировать текст, не отходя от смысла. "
        "Пиши кратко, но понятно. "
        "Если Гайворонский и Сапин расходятся — укажи это явно. "
        "Если по какому-то аспекту данных в источниках нет — напиши [нет в источниках]. "
...
def main() -> None:
    ...
    parser.add_argument("--output",          help="Название модели")
    parser.add_argument(
        "--question-range",
    ...
    if args.output:
        output_path = ROOT / "output" / args.output
    else:
    ...
    try:
        ...
        logger.error(f"Индекс {coll_name!r} не найден — сначала запустите:  python scripts/2_build_index.py")
        sys.exit(1)
